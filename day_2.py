...
def find_possible_games(input):
    # Range ignores empty string at beginning and end of array
    games = input.split("\n")[1:-1]
    game_products = []
    ...
    for game in games:
        ...
        # Array of subsets like ['3 blue, 4 red', '1 red, 2 green, 6 blue', '2 green']
        rounds = game.split(": ")[1].split("; ")
        for round in rounds:
            # Array of individual dice information like ['3 blue', '4 red']
            dice = round.split(", ")
            for die in dice:
                quantity, color = int(die.split(" ")[0]), die.split(" ")[1]
                if quantity > current_game[color]:
                    current_game[color] = quantity

        game_products.append(prod(current_game.values()))
